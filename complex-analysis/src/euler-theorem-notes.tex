...
\[
  i := (0,1).
\]
After defining multiplication on ordered pairs, we will verify that
\[
  i^{2} = -1.
\]
Thus the notation $i=\sqrt{-1}$ is a useful shorthand, but the rigorous construction starts from the ordered pair $(0,1)$.
\end{definition}
...
\[
  \C := \{(a,b): a,b\in\R\}.
\]
We write $(a,b)$ in the form
\[
  a+bi.
\]
Under this notation, a real number $a\in\R$ is identified with the complex number $a+0i$.
\end{definition}
...
Associativity, commutativity, and distributivity follow by direct expansion from the corresponding real-number laws.
If $z=a+bi$, then the additive inverse is
\[
  -z := (-a)+(-b)i.
\]
If $z\ne0$, then $a^{2}+b^{2}>0$ and the multiplicative inverse is
\[
  z^{-1}:=\frac{a}{a^{2}+b^{2}}-\frac{b}{a^{2}+b^{2}}i.
\]
A direct multiplication gives $zz^{-1}=1$.
\end{proof}
...
\[
  ...
  \qquad
  \sin\theta
  :=\sum_{n=0}^{\infty}(-1)^{n}\frac{\theta^{2n+1}}{(2n+1)!}.
\]
\end{definition}
...
\]
This series is absolutely convergent by the previous proposition, so its terms may be reordered and grouped without changing the sum.
...
\begin{proof}
Using the exponential addition law and Euler's formula,
\[
  \exp(z)=\exp(a+ib)=\exp(a)\exp(ib)=\exp(a)(\cos b+i\sin b).
\]
Since $\exp(a)$, $\cos b$, and $\sin b$ are real,
\[
...
\newpage
\section*{Useful Real and Imaginary Parts}
...
\conj{z}=a-bi & a & -b \\
z+w,\ w=c+di & a+c & b+d \\
zw,\ w=c+di & ac-bd & ad+bc \\
z^{-1},\ z\ne0 & \dfrac{a}{a^{2}+b^{2}} & -\dfrac{b}{a^{2}+b^{2}} \\
\dfrac{w}{z},\ z\ne0 & \dfrac{ac+bd}{a^{2}+b^{2}} & \dfrac{ad-bc}{a^{2}+b^{2}} \\
\exp(a+bi) & \exp(a)\cos b & \exp(a)\sin b \\
\exp(i\theta) & \cos\theta & \sin\theta \\
r e^{i\theta},\ r>0 & r\cos\theta & r\sin\theta
\end{array}
\]
